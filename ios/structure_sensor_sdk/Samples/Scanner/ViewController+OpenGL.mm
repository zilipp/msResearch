...
- (void)setupGLViewport
{
    ...
    CGFloat width = frameBufferSize.width;
    CGFloat height = frameBufferSize.height;

    float aspect = 0.75;
    float overflow = fabs((width * aspect - height) /2.0);
    _display.viewport[0] = 0;
    _display.viewport[1] = 0 - overflow;
    _display.viewport[2] = width;
    _display.viewport[3] = height + (overflow * 2.0);
